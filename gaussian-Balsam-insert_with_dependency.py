...
from balsam.launcher.dag import add_job, add_dependency
...
def main():
    ...
    for comFile in comFiles:
        ...
        parent.save()
        print('new job', parent.cute_id, 'will run in:', workdir)

        comName = comName.split('-')
        comName = comName[0] + "-reduced-" + comName[-1]
        workdir = top_dir
        child = new_job(
              name=comName,
              workdir=workdir,
        ...
        child.save()
        print('new job', child.cute_id, 'will run in:', workdir)
        print("Adding depenedcy")
